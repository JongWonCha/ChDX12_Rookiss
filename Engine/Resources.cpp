...
shared_ptr<Texture> Resources::CreateTexture(const wstring& name, DXGI_FORMAT format, uint32 width, uint32 height, const D3D12_HEAP_PROPERTIES& heapProp, D3D12_HEAP_FLAGS heapFlags, D3D12_RESOURCE_FLAGS resFlags, Vec4 clearColor)
{
	shared_ptr<Texture> texture = make_shared<Texture>();
	texture->Create(format, width, height, heapProp, heapFlags, resFlags, clearColor);
	Add(name, texture);
...
void Resources::CreateDefaultShader()
{
	...
	// Texture (Forward)
	{
		...
		shared_ptr<Shader> shader = make_shared<Shader>();
		shader->Init(L"..\\Resources\\Shader\\forward.hlsl", info, "VS_Tex", "PS_Tex");
	...
	// DirLight
	{
		...
		shared_ptr<Shader> shader = make_shared<Shader>();
		shader->Init(L"..\\Resources\\Shader\\lighting.hlsl", info, "VS_DirLight", "PS_DirLight");
	...
	// PointLight
	{
		...
		shared_ptr<Shader> shader = make_shared<Shader>();
		shader->Init(L"..\\Resources\\Shader\\lighting.hlsl", info, "VS_PointLight", "PS_PointLight");
	...
	// Final
	{
		...
		shared_ptr<Shader> shader = make_shared<Shader>();
		shader->Init(L"..\\Resources\\Shader\\lighting.hlsl", info, "VS_Final", "PS_Final");
...
void Resources::CreateDefaultMaterial()
{
	...
	// DirLight
	{
		...
		shared_ptr<Material> material = make_shared<Material>();
		material->SetShader(shader);
		shared_ptr<Texture> t0 = GET_SINGLE(Resources)->Get<Texture>(L"PositionTarget_0");
		shared_ptr<Texture> t1 = GET_SINGLE(Resources)->Get<Texture>(L"NormalTarget_0");
		material->SetTexture(0, t0);
		material->SetTexture(1, t1);
		/*material->SetTexture(0, GET_SINGLE(Resources)->Get<Texture>(L"PositionTarget_0"));
		material->SetTexture(1, GET_SINGLE(Resources)->Get<Texture>(L"NormalTarget_0"));*/
	...
	// Final
	{
		...
		shared_ptr<Material> material = make_shared<Material>();
		material->SetShader(shader);
		material->SetTexture(0, GET_SINGLE(Resources)->Get<Texture>(L"DiffuseTarget_0"));
		material->SetTexture(1, GET_SINGLE(Resources)->Get<Texture>(L"DiffuseLightTarget_0"));
		material->SetTexture(2, GET_SINGLE(Resources)->Get<Texture>(L"SpecularLightTarget_0"));
